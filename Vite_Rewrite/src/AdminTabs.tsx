...
import ResultsLock from './ResultsLock';
import { 
...
export default function AdminTabs({ data, refresh }: { data: any, refresh: () => void }) {
                  ...
                  {data.users.map((u: any) => (
                    <tr key={u.id} className="hover:bg-slate-800/50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-right space-x-4 font-medium">
                        ...
                        >EDIT</button>
                        <button 
                          onClick={async () => { if(confirm('Delete user?')) { setLoading(true); await deleteUser(u.id); setLoading(false); refresh(); } }}
              ...
              {data.candidates.map((c: any) => (
                <div key={c.id} className="p-6 border border-slate-700 rounded-2xl bg-slate-900/50 flex flex-col justify-between hover:border-indigo-500/50 transition-colors group">
                  ...
                  <div className="flex space-x-4 text-sm font-bold uppercase tracking-wider pt-4 border-t border-slate-700/50">
                    ...
                    >Edit</button>
                    <button 
                      onClick={async () => { if(confirm('Delete candidate?')) { setLoading(true); await deleteCandidate(c.id); setLoading(false); refresh(); } }}
